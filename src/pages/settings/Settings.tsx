...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const tokenPayload = JSON.parse(atob(getToken()!.split(".")[1]));
  ...
  const handleRemoveAdmin = async (id: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${BASE_URL}/admin/settings/admins/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
  ...
  return (
    <>
            ...
            <SettingRow
              ...
              control={
                <Toggle
                  ...
                  onChange={() => toggleNotif("newDisputeAlerts")}
                />
              }
            />
            <SettingRow
              ...
              control={
                <Toggle
                  checked={notifications.newOfficeRegistrations}
                  onChange={() => toggleNotif("newOfficeRegistrations")}
                />
              }
            />
            <SettingRow
              ...
              control={
                <Toggle
                  ...
                  onChange={() => toggleNotif("dailyRevenueReport")}
                />
              }
            />
          </Card>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-[var(--text-muted)] mt-0.5 truncate">
                        {admin.email} · {t("settings.admin")}
                      </p>
                    </div>
                  </div>
                  {admin.isYou ? (
                    <Badge variant="blue">{t("settings.you")}</Badge>
                  ) : removingId === admin.id ? (
      ...
      <SaveToast visible={toastVisible} message={t("settings.savedSuccess")} />
    </>
  );
};

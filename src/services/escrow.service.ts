...
export const escrowService = {
    getTransactions: async (status: string) => {
        ...
        const response = await fetch(`${BASE_URL}/escrow/admin/all?${query}`, {
            headers: {
        ...
        const transactions: EscrowTransaction[] = data.data.transactions.map(
            (t: any) => ({
                id: t._id,
                shipmentId: t.shipment?._id || t.shipment || "",
                trackingNumber:
                    t.shipment?.trackingNumber || t.trackingNumber || "N/A",
                customerName: t.customer?.fullName || t.customer || "N/A",
                driverName: t.driver?.fullName || t.driver || "N/A",
                amount: t.amount,
                status: t.status,
    ...
    releaseTransaction: async (id: string) => {
        const response = await fetch(`${BASE_URL}/escrow/admin/${id}/release`, {
            method: "PATCH",
            headers: {
                Authorization: `Bearer ${getToken()}`,
            },
    ...
    refundTransaction: async (id: string) => {
        const response = await fetch(`${BASE_URL}/escrow/admin/${id}/refund`, {
            method: "PATCH",
            headers: {
                Authorization: `Bearer ${getToken()}`,
            },
        ...
        if (!response.ok) {
            throw new Error(data.message || "Failed to refund escrow");
        }
